...
const runtimeRoot = process.env.CLAUDE_PLUGIN_DATA
  ...
  : null;
const sessionMarker = `openai-chrome-session-${randomUUID()}`;
const runtimeDir = runtimeRoot
  ? join(runtimeRoot, sessionMarker)
  : await mkdtemp(join(os.tmpdir(), 'openai-chrome-'));
const sessionId = `claude-openai-chrome-${randomUUID()}`;
...
function hostRequest(method, params, timeoutMs = 120000) {
  const id = `openai-chrome-${nextHostRequestId++}`;
...
async function handleChildRequest(request) {
  ...
  if (request.method === 'sampling/createMessage') {
    throw new Error('OpenAI Chrome never delegates browser work to another model');
  }
...
async function ensureChild() {
  ...
  try {
    ...
    if (!childTools.some(tool => tool.name === 'js') || !childTools.some(tool => tool.name === 'js_reset')) {
      throw new Error('Installed OpenAI node_repl is missing js/js_reset');
    }
    child = candidate;
    return child;
  } catch (cause) {
  ...
  try {
    if (method === 'initialize') {
      ...
      result(id, {
        ...
        serverInfo: { name: 'openai-chrome', version: '0.1.0' },
        instructions: 'UI automation through a persistent JavaScript session using the initialized OpenAI Chrome CUA API.',
      });
      return;
    }
    ...
    if (method === 'tools/call') {
      if (!['js', 'js_reset', 'turn_ended'].includes(params.name)) {
        error(id, -32602, `Unknown OpenAI Chrome tool: ${params.name}`);
        return;
      }
...
async function shutdown() {
  if (stopping) return;
  stopping = true;
  for (const waiter of hostRequests.values()) waiter.reject(new Error('OpenAI Chrome proxy stopped'));
  hostRequests.clear();
  if (child) {
