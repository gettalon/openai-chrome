...
export class ChildMcp {
  ...
  async start() {
    ...
    const [banner, baseDescription, browserDescription, outputDescription, resetDescription] = await Promise.all([
      readFile(join(cuaResources, 'banner-browser.js'), 'utf8'),
    ...
    const env = {
      ...
      NODE_REPL_TOOL_OVERRIDES: JSON.stringify({
        server_instructions: 'UI automation through a persistent JavaScript session using the initialized OpenAI Chrome CUA API.',
        tools: {
          js: {
            description: [baseDescription, browserDescription, outputDescription].join('\n\n'),
            field_descriptions: { code: 'JavaScript to execute using the initialized OpenAI Chrome CUA runtime.' },
          },
    ...
    await this.request('initialize', {
      ...
      clientInfo: { name: 'claude-openai-chrome', version: '0.1.0' },
    });
