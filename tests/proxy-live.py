...
SERVER = Path(__file__).resolve().parents[1] / "skills" / "openai-chrome" / "scripts" / "proxy-server.mjs"
...
try:
    ...
    send({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"js","arguments":{"code":"await cua.getState();","timeout_ms":30000,"title":"Read OpenAI Chrome state"}}})
    state = receive(3)
    assert not state["result"].get("isError"), state
    text = "\n".join(x["text"] for x in state["result"]["content"] if x["type"] == "text")
    assert '"family":"chrome"' in text or 'family: "chrome"' in text, text
    ...
    print(json.dumps({"ok":True,"metadata":"synthesized","browser":"chrome","turnEnded":"ok"}))
finally:
    p.terminate()
